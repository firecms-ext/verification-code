 ...
 */
namespace FirecmsExt\VerificationCode\Services;
...
class VerificationCodeService implements VerificationCodeServiceInterface
{
    ...
    // 缓存 key
    private string $cache_key = 'verification_code_blacklist:';
    ...
    public function check(string $code, string $key): bool
    {
        ...
        if ($res) {
            ...
            $res = ! $this->cache->has($cache_key);
            if ($res && $ttl = $data['expired_at'] - $datetime) {
                // 加入黑名单
